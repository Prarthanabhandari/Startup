  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Server returned an error. Submitting with local fallback...');
        setIsSubmitted(true);
      }
    } catch (error) {
      console.log('Backend not reachable, executing local fallback.');
      setIsSubmitted(true);
      
      // Celebrate with confetti anyway
      confetti({
        particleCount: 150,
        spread: 80,
        origin: { y: 0.6 },
          ...
          <div className="lg:col-span-5 space-y-8 order-2 lg:order-1">
            ...
            <div className="space-y-4 pt-4 hidden sm:block">
              <div className="flex items-center space-x-3 text-slate-400 text-xs">
                ...
                <span>projects@voxorlab.tech</span>
              </div>
              <div className="flex items-center space-x-3 text-slate-400 text-xs">
